...
- **D-Bus interface**: `org.rsynapse.Engine1` on service `org.rsynapse.Engine`, object path `/org/rsynapse/Engine1`. Methods: `Search(query) -> Vec<(id, title, description, icon, command)>`, `Execute(id)` (WIP).
